...
public class Main {
  ...
  private void run() throws Exception {
    ...
    if (CONFIGURATION_MAP.hasError()) {
      LOGGER.severe("Error with configuration, stopping program.");
      System.exit(-1);
    }
    ...
    Pohc POHC = new Pohc(
        CONFIGURATION_MAP.get("pohc.host"),
    ...
    PohcView POHC_VIEW = new PohcView(
        CONFIGURATION_MAP.get("tnpm.host"),
        CONFIGURATION_MAP.get("tnpm.db.port"),
        CONFIGURATION_MAP.get("tnpm.db.schema"),
        CONFIGURATION_MAP.get("tnpm.db.user"),
    ...
    int BATCH_SIZE = Integer.parseInt(CONFIGURATION_MAP.get("config.batch.size"));
    int TOTAL_ITERATIONS = 
        (POHC.getTotaNumberOfRows() / BATCH_SIZE) + 1;
    LOGGER.info(String.format("About to process %d iteration(s).", TOTAL_ITERATIONS));
    
    for (int ITERATION = 0; ITERATION < TOTAL_ITERATIONS; ITERATION++) {
      ArrayList<PohcDefinition> POHC_DEFINITIONS = 
          POHC.getPohcRecords(BATCH_SIZE, BATCH_SIZE * ITERATION);
      ...
      LOGGER.info(
          String.format("About to further process %d objects.", 
      ...
      LOGGER.info(String.format("config.commit=%s", CONFIGURATION_MAP.get("config.commit")));
      ...
      //dumpPohcDefinition(EXPLODED_POHC_DEFINITIONS);
    }
  }
